...
static void CheckUPCReferenceType(Sema &S, Expr *&E) { // Cray UPC
  ...
  if (Quals.hasShared() && !Quals.hasStrict() && !Quals.hasRelaxed()) {
    ...
    E = ImplicitCastExpr::Create(
      ...
      CK_LValueBitCast, E, 0, VK_LValue, FPOptionsOverride());
  }
}

static void CheckUPCReferenceType(Sema &S, ExprResult &E) { // Cray UPC
  Expr *Temp = E.get();
...
ExprResult Sema::DefaultLvalueConversion(Expr *E) {
  ...
  // Cray UPC - Mark shared access as either strict or relaxed
  if (getLangOpts().UPC)
...
ExprResult Sema::ActOnUPCThreadsExpr(SourceLocation Loc) { // Cray UPC
  return new (Context) UPCThreadExpr(Loc, Context.IntTy);
}

ExprResult Sema::ActOnUPCMyThreadExpr(SourceLocation Loc) { // Cray UPC
...
static bool CheckExtensionTraitOperandType(Sema &S, QualType T,
  ...
  if (T->isFunctionType() &&
    ...
    switch (TraitKind) {
    case UETT_SizeOf:
    case UETT_UPC_LocalSizeOf: // Cray UPC
    case UETT_UPC_BlockSizeOf: // Cray UPC
    case UETT_UPC_ElemSizeOf: // Cray UPC
      S.Diag(Loc, diag::ext_sizeof_alignof_function_type) << TraitKind << ArgRange;
      break;
    default: break;
    }
    return false;
...
bool Sema::CheckUnaryExprOrTypeTraitOperand(Expr *E,
  ...
  // Cray UPC - Can only be appied to shared qualified types
  switch (ExprKind) {
  ...
  case UETT_UPC_ElemSizeOf:
    if (!ExprTy.getQualifiers().hasShared()) {
      Diag(E->getExprLoc(), diag::err_upc_xsizeof_applied_to_non_shared)
        << getUPCSizeofDiagIndex(ExprKind) << E->getSourceRange();
      return true;
    }
  default: break;
  }
...
static bool CheckAlignOfExpr(Sema &S, Expr *E, UnaryExprOrTypeTrait ExprKind) {
  ...
  if (E->getObjectKind() == OK_BitField) {
    S.Diag(E->getExprLoc(), diag::err_sizeof_alignof_typeof_bitfield)
       << UETT_AlignOf << E->getSourceRange(); // Cray UPC
    return true;
  }
...
#include "clang/AST/TypeNodes.inc"
    ...
    case Type::UPCThreadArray: // Cray UPC
  ...
  case UETT_UPC_ElemSizeOf:
    if (!ExprType.getQualifiers().hasShared()) {
      Diag(OpLoc, diag::err_upc_xsizeof_applied_to_non_shared)
        << getUPCSizeofDiagIndex(ExprKind) << ExprRange;
      return true;
    }
  default: break;
  }
...
Sema::CreateUnaryExprOrTypeTraitExpr(Expr *E, SourceLocation OpLoc,
  ...
  if (E->isTypeDependent()) {
    ...
    Diag(E->getExprLoc(), diag::err_sizeof_alignof_typeof_bitfield) << ExprKind; // Cray UPC
    isInvalid = true;
  } else if (ExprKind == UETT_VectorElements) {
    isInvalid = CheckUnaryExprOrTypeTraitOperand(E, UETT_VectorElements);
  } else {
    isInvalid = CheckUnaryExprOrTypeTraitOperand(E, ExprKind); // Cray UPC
  }
...
// Cray UPC
static CastKind CheckPointerToSharedConversion(
    ASTContext& Context, QualType SrcPointee, QualType DestPointee) {
  // UPC 1.2 6.4.3p3
...
CastKind Sema::PrepareScalarCast(ExprResult &Src, QualType DestTy) {
  ...
  case Type::STK_UPCSharedPointer: // Cray UPC
...
static QualType checkConditionalPointerCompatibility(Sema &S, ExprResult &LHS,
  ...
  if (lhQual.hasShared() != rhQual.hasShared()) { // Cray UPC
  ...
  if (CompositeTy.isNull()) {
    ...
    QualType Pointee = S.Context.getAddrSpaceQualType(
        S.Context.VoidTy, ResultAddrSpace);
    if (lhQual.hasShared()) { // Cray UPC
      Qualifiers Quals;
      Quals.addShared();
...
Sema::CheckAssignmentConstraints(QualType LHSType, ExprResult &RHS,
  ...
  if (const PointerType *LHSPointer = dyn_cast<PointerType>(LHSType)) {
    ...
    if (const PointerType *RHSPointer = dyn_cast<PointerType>(RHSType)) {
      ...
      LangAS AddrSpaceL = LHSPointee.getAddressSpace();
      LangAS AddrSpaceR = RHSPointee.getAddressSpace();
      if (AddrSpaceL != AddrSpaceR)
        Kind = CK_AddressSpaceConversion;
      else if (RHSPointee.getQualifiers().hasShared())
    ...
    if (RHSType->isIntegerType()) {
      ...
      if (LHSPointer->getPointeeType().getQualifiers().hasShared()) // Cray UPC
        return Incompatible;
      else
        return IntToPointer;
    }
...
static void diagnoseArithmeticOnTwoVoidPointers(Sema &S, SourceLocation Loc,
  ...
  S.Diag(Loc, (S.getLangOpts().CPlusPlus ||
               LHSExpr->getType()->getAs<PointerType>()->getPointeeType().getQualifiers().hasShared()) // Cray UPC
...
static void diagnoseArithmeticOnVoidPointer(Sema &S, SourceLocation Loc,
                                            Expr *Pointer) {
  S.Diag(Loc, (S.getLangOpts().CPlusPlus ||
               Pointer->getType()->getAs<PointerType>()->getPointeeType().getQualifiers().hasShared()) // Cray UPC
                ? diag::err_typecheck_pointer_arith_void_type
                : diag::ext_gnu_void_ptr)
  ...
  if (LHS.get()->getType()->isAnyPointerType()) {
    ...
    if (const PointerType *RHSPTy
      ...
      if (getLangOpts().CPlusPlus) {
        ...
        if (LQuals.hasShared() != RQuals.hasShared() || // Cray UPC
      ...
      if (!rpointee->isVoidType() && !rpointee->isFunctionType() &&
          !rpointee->isUPCThreadArrayType()) { // Cray UPC
...
QualType Sema::CheckCompareOperands(ExprResult &LHS, ExprResult &RHS,
  ...
  if ((LHSType->isIntegerType() && !LHSIsNull) ||
    ...
    if ((LQuals.hasShared() != RQuals.hasShared() ||
      ...
      // Cray UPC - Relational comparison of pointers-to-shared requires a
      // complete type
      ...
      if ((LCanPointeeTy->isFunctionType() || RCanPointeeTy->isFunctionType())
        ...
        diagnoseFunctionPointerToVoidComparison(*this, Loc, LHS, RHS,
                                                /*isError*/false);
    } else if (IsRelational && LQuals.hasShared()) { // Cray UPC
      diagnoseDistinctPointerComparison(*this, Loc, LHS, RHS, /*isError*/true);
    } else {
      // Invalid
    ...
    if (LCanPointeeTy != RCanPointeeTy) {
      ...
      if (LHSIsNull && !RHSIsNull) {
        ...
        if (LQuals.hasShared() && !RQuals.hasShared() && RHSIsNull) // Cray UPC
          RHS = ImpCastExprToType(RHS.get(), LHSType, CK_NullToPointer);
        else
...
// C99 6.5.16.1
QualType Sema::CheckAssignmentOperands(ExprResult &LHS, ExprResult &RHS, // Cray UPC
                                       SourceLocation Loc,
                                       QualType CompoundType,
                                       BinaryOperatorKind Opc) {
  assert(!LHS.get()->hasPlaceholderType(BuiltinType::PseudoObject)); // Cray UPC

  // Verify that LHS is a modifiable lvalue, and emit error if not.
  if (CheckForModifiableLvalue(LHS.get(), Loc, *this)) // Cray UPC
    return QualType();

  QualType LHSType = LHS.get()->getType(); // Cray UPC
  ...
  if (CompoundType.isNull()) {
    ...
    CheckIdentityFieldAssignment(LHS.get(), RHSCheck, Loc, *this); // Cray UPC
    ...
    if (ConvTy == Compatible) {
      if (LHSType.getObjCLifetime() == Qualifiers::OCL_Strong) {
        ...
        const Expr *InnerLHS = LHS.get()->IgnoreParenCasts(); // Cray UPC
        const DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(InnerLHS);
        if (!DRE || DRE->getDecl()->hasAttr<BlocksAttr>())
          ObjC().checkRetainCycles(LHS.get(), RHS.get()); // Cray UPC
      }
      ...
      if (LHSType.getObjCLifetime() == Qualifiers::OCL_Strong ||
        ...
        checkUnsafeExprAssigns(Loc, LHS.get(), RHS.get()); // Cray UPC
      }
    }
  } else {
  ...
  CheckForNullPointerDereference(*this, LHS.get()); // Cray UPC

  if (getLangOpts().UPC) // Cray UPC
    CheckUPCReferenceType(*this, LHS);

  AssignedEntity AE{LHS.get()}; // Cray UPC
  ...
  if (getLangOpts().CPlusPlus20 && LHSType.isVolatileQualified()) {
    if (CompoundType.isNull()) {
      ...
      ExprEvalContexts.back().VolatileAssignmentLHSs.push_back(LHS.get());  // Cray UPC
    }
  }
...
ExprResult Sema::CreateBuiltinBinOp(SourceLocation OpLoc,
  ...
  switch (Opc) {
  case BO_Assign:
    ResultTy = CheckAssignmentOperands(LHS /* Cray UPC */, RHS, OpLoc, QualType(), Opc);
    if (getLangOpts().CPlusPlus &&
  ...
  case BO_DivAssign:
    ...
    if (!CompResultTy.isNull() && !LHS.isInvalid() && !RHS.isInvalid())
      ResultTy =
          CheckAssignmentOperands(LHS /* Cray UPC */, RHS, OpLoc, CompResultTy, Opc);
    break;
  case BO_RemAssign:
...
bool Sema::DiagnoseAssignmentResult(AssignConvertType ConvTy,
  ...
  case IncompatiblePointerDiscardsQualifiers: {
    ...
    if (lhq.getAddressSpace() != rhq.getAddressSpace()) {
      ...
      break;
    } else if (lhq.hasShared() && !rhq.hasShared()) { // Cray UPC
...
ExprResult Sema::CheckPlaceholderExpr(Expr *E) {
  ...
  case BuiltinType::BuiltinFn: {
    ...
    if (DRE) {
      ...
      else if (getLangOpts().Cray_SuppressUncalledBuiltinError) { // Cray
